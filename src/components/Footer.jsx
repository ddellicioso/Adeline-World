    ...
    <>
      {pathName.pathname === "/" && (
        <div className="mt-[-5rem] lg:mt-[12rem] mx-[auto] w-[100%]">
          <div className="md:flex justify-around items-center lg:my-[3rem] mx-[auto] w-fit md:w-[100%]">
            <div>
              <h1 className="h1-2 text-[6rem] md:text-[7.5rem] lg:text-[10rem]">
                Let's Get
              </h1>
              <h1 className="h1-1 mt-[1.5rem] text-[4.5rem] md:text-[5.8rem] md:mt-[2.5rem] lg:text-[7.5rem] lg:mt-[4rem] mb-[5rem]">
                In Touch!
              </h1>
            </div>
            <div className="mt-[-3.5rem] mb-[2.5rem] md:mt-[-2rem] lg:mt-[-2.3rem]">
              <GlassContainer className="mb-[2rem] rounded-[1rem]">
                <a
                  className="flex items-center"
                  href="mailto:tan.ellyshia@gmail.com"
                >
                  <MailIcon className="w-[1.5rem] m-[1rem] mr-0" />
                  <p className="m-[1rem]">tan.ellyshia@gmail.com</p>
                </a>
              </GlassContainer>
              <ul className="hidden md:flex flex-col items-end font-interItalic md:mt-[2rem] lg:mt-[5rem]">
                {socials.map((item) => (
                  <li className="my-[.8rem]">
                    <a
                    ...
                    >
                      {item.title === "Linkedin" && (
                        <LinkedinIcon className="w-[1.5rem] opacity-[.9] group-hover:opacity-[1] transition-all" />
                      )}
                      {item.title === "Gitlab" && (
                        <GitlabIcon className="w-[1.5rem] opacity-[.9] group-hover:opacity-[1] transition-all" />
                      )}
                      <p className="relative text-[15px] ml-[.4rem] after:content-[''] after:opacity-0 hover:after:opacity-[.7] after:w-[100%] after:transition-all after:duration-250 after:h-[1.1px] after:bg-color-2 after:absolute after:left-0 after:bottom-[.2rem]">
                        {item.title}
                      </p>
                      <LinkArrow className="w-[.7rem] h-[.7rem] ml-[.3rem] opacity-[.8] group-hover:opacity-[1]" />
                    </a>
                  </li>
                ))}
              </ul>
      ...
      {/* {pathName.pathname.includes("WorkDetailPage") && (
        <div className="flex flex-col items-center w-[100%] bg-color-7">
          <div className="flex flex-col items-center w-[50%] h-[7rem] text-white mt-[2rem]">
            <h4 className="font-instrumentSerifItalic text-[2.2rem]">
              Done reading?
            </h4>
            <ul class="flex justify-between w-[100%] my-[2rem]">
              <li> */}
      {/* Arrow to previous work in the array */}
      {/* <Link to={`/WorksPage/${backLink}/WorkDetailPage/${WorksList[previousIndex].url}`} className="flex items-center">
                  <Arrow
                    white={true}
                    className="w-[1rem] rotate-[180deg] mx-[.4rem]"
                  />
                  <p>Previous work</p>
                </Link>
              </li>
              <li>
                <Link to={`/WorksPage/${backLink}`}>
                  <p>Back to list</p>
                </Link>
              </li>
              <li> */}
      {/* Arrow to next work in the array */}
      {/* <Link to={`/WorksPage/${backLink}/WorkDetailPage/${WorksList[nextIndex].url}`} className="flex items-center">
                  <p>Next work</p>
                  <Arrow white={true} className="w-[1rem] mx-[.4rem]" />
                </Link>
              </li>
            </ul>
          </div>
          ...
          <div className="flex justify-between mx-[1rem] my-[.4rem] w-[100%]">
            <p className="text-[10px] uppercase mx-[1rem] text-white font-interItalic">
              All rights reserved Ellyshia Cheryl Tan 2024
            </p>
            <ul className="flex justify-between items-center font-interItalic scale-[.9] w-[25%]">
              {socials.map((item) => (
                <a href={item.url} key={item.id}>
